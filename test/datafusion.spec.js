...
describe('Datafusion', () => {
  ...
  const SQL_LIST = [
    ...
    },
    {
      title: 'with or without timezone',
      sql: [
        'select try_cast(c as time with time zone)',
        'SELECT TRY_CAST(c AS TIME WITH TIME ZONE)'
      ]
    },
    {
      title: 'with or without timezone',
      sql: [
        'select try_cast(c as timestamp without time zone)',
        'SELECT TRY_CAST(c AS TIMESTAMP WITHOUT TIME ZONE)'
      ]
    },
    {
    ...
    },
    {
      ...
      sql: [
        `CREATE FUNCTION dup(int) RETURNS TABLE(f1 int, f2 text)
        AS $$ SELECT $1, TRY_CAST($1 AS text) || ' is text' $$
        LANGUAGE SQL;`,
        `CREATE FUNCTION dup(INT) RETURNS TABLE (f1 INT, f2 TEXT) AS $$ SELECT $1, TRY_CAST($1 AS TEXT) || ' is text' $$ LANGUAGE SQL`
      ]
    },
    {
